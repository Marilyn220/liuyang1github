# -*- coding: utf-8 -*-
# python 2.7
# @Time    : 2019/1/25 15:20
# @Author  : Chengzy
# @File    : Logs.py
# @Software: PyCharm
#
# import logging
# import os
# import time


# class Log:
#     def __init__(self):
#
#         log_file = +'%s.log' % time.strftime('%Y.%m.%d',time.localtime(time.time()))
#         print log_file
#
#         self.logger = logging.getLogger('logger')
#         self.logger.setLevel(logging.DEBUG)
#         #os.mkdir(log_file)
#
#         fh = logging.FileHandler(log_file, mode='a+')
#         ch = logging.StreamHandler()
#
#         formatter = logging.Formatter('%(asctime)s - %(name)s - %(lineno)d - %(levelname)s - %(message)s')
#
#         fh.setFormatter(formatter)
#         ch.setFormatter(formatter)
#
#         self.logger.addHandler(fh)
#         self.logger.addHandler(ch)
#         # self.logger.removeHandler(fh)
#
#     def getlog(self):
#         return self.logger
#     @staticmethod
#     def p(path):
#         return path



import os
import os.path
import socket
import logging
import logging.handlers
logging.basicConfig()


def singleton(cls, *args, **kw):
    instances = {}

    def _singleton():
        if cls not in instances:
            instances[cls] = cls(*args, **kw)
        return instances[cls]
    return _singleton



class JFMlogging(object):
    logger = logging.getLogger()

    def __init__(self,log_path):
        host_name = socket.gethostname()
        # ip = socket.gethostbyname(host_name)
        logging_msg_format = '[%(asctime)s] [%(levelname)s] [' + host_name + '][%(module)s.py - line:%(lineno)d] %(message)s'
        logging_date_format = '%Y-%m-%d %H:%M:%S'
        #log_path = 'logs'  # 日志存放目录
        logging.basicConfig(level=logging.INFO, format=logging_msg_format, datefmt=logging_date_format)
        self.logger.setLevel(logging.INFO)

        if not os.path.exists(log_path):
            os.mkdir(log_path)
        log_file = os.path.join(log_path, 'system.log')

        fileHandler = logging.handlers.TimedRotatingFileHandler(log_file, 'midnight', 1)
        fileHandler.setFormatter(logging.Formatter(logging_msg_format))
        self.logger.addHandler(fileHandler)

    def getloger(self):
        return self.logger



if __name__=="__main__":
    logger = JFMlogging('D:\chengzhengyang\PycharmProjects\QBXD3.1\TestCase\Product01\chengzy').getloger()
    logging.info('aaaaaaaaaa')