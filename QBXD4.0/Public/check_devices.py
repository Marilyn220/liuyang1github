# -*- coding: utf-8 -*-
# python 2.7
# @Time    : 2019/1/25 16:21
# @Author  : Chengzy
# @File    : check_devices.py
# @Software: PyCharm
import re,os
import subprocess
...
class check_devices:
    ...
    def connect_devices(self,dr_lists):
        ...
        if valid_serials==[]:
            ...
            #print 'adb devices: %s' % output
            output = Popen("adb devices",shell=True,stdout=PIPE,stderr=PIPE)
            #print ('adb devices: %s' % output)
            pattern = re.compile(
                r'(?P<serial>[^\s]+)\t(?P<status>device|offline)')
            matches = pattern.findall(output.decode())
